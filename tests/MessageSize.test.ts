  ...
  it("send invocation", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      try {
        ...
        hubConnection.send("target", 1).catch((_) => {}); // Suppress exception and unhandled promise rejection warning.

        // Verify the message is sent
        expect(connection.sentData.length).toBe(1);
        expect(connection.parsedSentData[0].type).toEqual(
  ...
  it("invoke invocation", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      try {
        ...
        // tslint:disable-next-line:no-floating-promises
        hubConnection.invoke("target", 1).catch((_) => {}); // Suppress exception and unhandled promise rejection warning.
  ...
  it("upload invocation", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      try {
        // We don't actually care to wait for the invoke.
        // tslint:disable-next-line:no-floating-promises
        hubConnection.invoke("target", 1, new Subject()).catch((_) => {}); // Suppress exception and unhandled promise rejection warning.
  ...
  it("completion message", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      try {
        ...
        expect(connection.parsedSentData[1].type).toEqual(
