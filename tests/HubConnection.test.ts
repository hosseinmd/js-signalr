    ...
    it("automatically sends multiple pings", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          const numPings = connection.sentData.filter(
            (s) => JSON.parse(s).type === MessageType.Ping,
          ).length;
    ...
    it("sends a non blocking invocation", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          hubConnection.send("testMethod", "arg", 42).catch((_) => {}); // Suppress exception and unhandled promise rejection warning.
    ...
    it("works if argument is null", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          // We don't actually care to wait for the send.
          // tslint:disable-next-line:no-floating-promises
          hubConnection.send("testMethod", "arg", null).catch((_) => {}); // Suppress exception and unhandled promise rejection warning.
  ...
  describe("invoke", () => {
    it("sends an invocation", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          // We don't actually care to wait for the send.
          // tslint:disable-next-line:no-floating-promises
          hubConnection.invoke("testMethod", "arg", 42).catch((_) => {}); // Suppress exception and unhandled promise rejection warning.
    ...
    it("can process handshake and additional messages from binary", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        const mockProtocol = new TestProtocol(TransferFormat.Binary);
        mockProtocol.onreceive = (d) =>
          (receivedProcotolData = d as ArrayBuffer);
    ...
    it("can process handshake and additional messages from text", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          expect(receivedProcotolData).toEqual(
    ...
    it("is able to send stream items to server with invoke", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          const invokePromise = hubConnection.invoke(
            "testMethod",
            "arg",
            subject,
          );
      ...
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          expect(warnings).toEqual([
            "No client method with the name 'message' found.",
          ]);
        } finally {
          await hubConnection.stop();
        }
      ...
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          expect(closeError!.message).toEqual(
            "Server returned an error on close: Error!",
          );
        } finally {
          await hubConnection.stop();
        }
    ...
    it("completes pending streams when stopped", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          const observer = new TestObserver();
          hubConnection.stream<any>("testMethod").subscribe(observer);
          await hubConnection.stop();

          await expect(observer.completed).rejects.toThrow(
    ...
    it("can be canceled", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        try {
          ...
          const observer = new TestObserver();
          const subscription = hubConnection
            .stream("testMethod")
            .subscribe(observer);
