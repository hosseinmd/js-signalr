...
describe("HubConnectionBuilder", () => {
  for (const val of [undefined, null, ""]) {
    it(`withUrl throws if url is ${String(val)}`, () => {
      ...
      expect(() => builder.withUrl(val!)).toThrow(
        /The 'url' argument (is required|should not be empty)./,
      );
    });
  }
  ...
  eachMissingValue((val, name) => {
    it(`withHubProtocol throws if protocol is ${name}`, () => {
      const builder = new HubConnectionBuilder();
      expect(() => builder.withHubProtocol(val!)).toThrow(
  ...
  it("builds HubConnection with HttpConnection using provided URL", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      const testClient = createTestClient(pollSent, pollCompleted.promise).on(
        "POST",
      ...
      expect((await pollSent.promise).url).toMatch(
        /http:\/\/example.com\?id=123abc.*/,
      );
    });
  });
  ...
  it("can configure transport type", async () => {
    ...
    expect(builder.httpConnectionOptions!.transport).toBe(
  ...
  it("can configure hub protocol", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      expect(negotiateRequest.content).toBe(
  ...
  describe("configureLogging", () => {
    ...
    eachMissingValue((val, name) => {
      it(`throws if logger is ${name}`, () => {
        const builder = new HubConnectionBuilder();
        expect(() => builder.configureLogging(val!)).toThrow(
          "The 'logging' argument is required.",
        );
      });
    });
    ...
    [
      ...
      it(`accepts LogLevel.${levelName}`, async () => {
        const builder = new HubConnectionBuilder().configureLogging(minLevel);
    ...
    const levelNames = Object.keys(ExpectedLogLevelMappings) as Array<
      keyof typeof ExpectedLogLevelMappings
    >;
    for (const str of levelNames) {
      ...
      it(`accepts "${str}" as an alias for LogLevel.${mappedName}`, async () => {
        const builder = new HubConnectionBuilder().configureLogging(str);
    ...
    it("does not replace HttpConnectionOptions logger if provided", async () => {
      ...
      expect(httpConnectionLogger.messages).not.toContain(
        "Starting HubConnection.",
      );

      // An HttpConnection message
      expect(httpConnectionLogger.messages).toContain(
        "Starting connection with transfer format 'Text'.",
      );
      expect(hubConnectionLogger.messages).not.toContain(
        "Starting connection with transfer format 'Text'.",
      );
    });
  });
  ...
  it("withAutomaticReconnect throws if reconnectPolicy is already set", () => {
    ...
    expect(() => builder.withAutomaticReconnect()).toThrow(
  ...
  it("withAutomaticReconnect uses a custom IRetryPolicy when provided", () => {
    ...
    const builder = new HubConnectionBuilder().withAutomaticReconnect(
      new DefaultReconnectPolicy(customRetryDelays),
    );
...
function createConnectionBuilder(logger?: ILogger): HubConnectionBuilder {
  ...
  return new HubConnectionBuilder().configureLogging(
    logger || NullLogger.instance,
  );
}

function createTestClient(
  pollSent: PromiseSource<HttpRequest>,
  pollCompleted: Promise<HttpResponse>,
...
function eachMissingValue(
  callback: (val: undefined | null, name: string) => void,
) {
  callback(null, "null");
