...
describe("ServerSentEventsTransport", () => {
  it("does not allow non-text formats", async () => {
    await VerifyLogger.run(async (logger) => {
      const sse = new ServerSentEventsTransport(
        new TestHttpClient(),
        undefined,
        logger,
        true,
        TestEventSource as any,
        true,
        {},
      );

      await expect(sse.connect("", TransferFormat.Binary)).rejects.toEqual(
        new Error(
  ...
  it("connect waits for EventSource to be connected", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestEventSource.eventSource.onopen(new TestMessageEvent() as any);
  ...
  it("connect failure does not call onclose handler", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestEventSource.eventSource.onerror(new TestMessageEvent() as any);

      await expect(connectPromise).rejects.toEqual(new Error("Error occurred"));
  ...
  it("sets Authorization header on sends", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      const sse = await createAndStartSSE(
        logger,
        "http://example.com",
        () => "secretToken",
        httpClient,
  ...
  it("can receive data", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestEventSource.eventSource.onmessage(message as any);
  ...
  it("can close from EventSource error", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestEventSource.eventSource.onerror(errorEvent as any);
  ...
  it("send throws if not connected", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      await expect(sse.send("")).rejects.toEqual(
  ...
  it("closes on error from receive", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestEventSource.eventSource.onmessage(errorEvent as any);
  ...
  it("sets user agent header on connect and sends", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      expect(
        (TestEventSource.eventSource.eventSourceInitDict as any).headers[
          `User-Agent`
        ],
      ).toEqual(value);
      await sse.send("");
...
async function createAndStartSSE(
  logger: ILogger,
  url?: string,
