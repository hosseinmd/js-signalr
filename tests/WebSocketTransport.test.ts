...
describe("WebSocketTransport", () => {
  ...
  it("connect waits for WebSocket to be connected", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestWebSocket.webSocket.onopen((new TestEvent() as unknown) as Event);
  ...
  it("connect fails if there is error during connect", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestWebSocket.webSocket.onerror((new TestEvent() as unknown) as Event);
  ...
  it("connect failure does not call onclose handler", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestWebSocket.webSocket.onclose((new TestEvent() as unknown) as Event);
  ...
  it("can receive data", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestWebSocket.webSocket.onmessage((message as unknown) as MessageEvent);
  ...
  it("is closed from WebSocket onclose with error", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestWebSocket.webSocket.onclose((message as unknown) as Event);

      expect(closeCalled).toBe(true);
      expect(error!).toEqual(
        new Error("WebSocket closed with status code: 1 (just cause)."),
      );

      await expect(webSocket.send("")).rejects.toBe(
  ...
  [
    ...
    it(`can send ${
      TransferFormat[format as TransferFormat]
    } data`, async () => {
      await VerifyLogger.run(async (logger) => {
        const webSocket = await createAndStartWebSocket(
          logger,
          "http://example.com",
          undefined,
  ...
  it("sets user agent header on connect", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      expect(TestWebSocket.webSocket.options!.headers[`User-Agent`]).toEqual(
        value,
      );
  ...
  it("overwrites library headers with user headers", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      expect(TestWebSocket.webSocket.options!.headers[`X-HEADER`]).toEqual(
        "VALUE",
      );
...
async function createAndStartWebSocket(
  logger: ILogger,
  url?: string,
