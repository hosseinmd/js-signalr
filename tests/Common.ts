...
export function eachEndpointUrl(
  action: (givenUrl: string, expectedUrl: string) => void,
) {
  const urls = [
    [
...
export class VerifyLogger implements ILogger {
  ...
  public constructor(
    ...expectedErrors: Array<RegExp | string | ErrorMatchFunction>
  ) {
  ...
  public static async run(
    fn: (logger: VerifyLogger) => Promise<void>,
    ...expectedErrors: Array<RegExp | string | ErrorMatchFunction>
  ): Promise<void> {
