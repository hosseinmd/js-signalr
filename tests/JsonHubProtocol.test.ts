...
describe("JsonHubProtocol", () => {
  it("can write/read non-blocking Invocation message", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      const parsedMessages = protocol.parseMessages(
  ...
  ] as Array<[string, CompletionMessage]>).forEach(
  ...
  ] as Array<[string, StreamItemMessage]>).forEach(
    ([payload, expectedMessage]) =>
  ...
  ] as Array<[string, string, string]>).forEach(
