...
describe("HttpConnection", () => {
  it("cannot be created with relative url if document object is not present", () => {
    expect(() => new HttpConnection("/test", commonOptions)).toThrow(
      "Cannot resolve '/test'.",
    );
  });
  ...
  it("starting connection fails if getting id fails", async () => {
    await VerifyLogger.run(
      async (logger) => {
        ...
        await expect(connection.start(TransferFormat.Text)).rejects.toBe(
          "error",
        );
      },
  ...
  it("cannot start a running connection", async () => {
    await VerifyLogger.run(async (logger) => {
      const options: IHttpConnectionOptions = {
        ...commonOptions,
        httpClient: new TestHttpClient().on(
          "POST",
          () => defaultNegotiateResponse,
        ),
        logger,
        transport: {
          connect() {
      ...
      try {
        ...
        await expect(connection.start(TransferFormat.Text)).rejects.toThrow(
  ...
  it("cannot send with an un-started connection", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      await expect(connection.send("LeBron James")).rejects.toThrow(
  ...
  it("cannot be started if negotiate returns non 200 response", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      const connection = new HttpConnection("http://tempuri.org", options);
      await expect(connection.start(TransferFormat.Text)).rejects.toThrow(
        "Unexpected status code returned from negotiate '999'",
      );
  ...
  it("all transport failure errors get aggregated", async () => {
    await VerifyLogger.run(
      async (loggerImpl) => {
        ...
        const connection = new HttpConnection("http://tempuri.org", options);
        await expect(connection.start(TransferFormat.Text)).rejects.toThrow(
          "Unable to connect to the server with any of the available transports. WebSockets failed: TypeError: this.webSocketConstructor is not a constructor ServerSentEvents failed: TypeError: this.eventSourceConstructor is not a constructor LongPolling failed: Error: 'LongPolling' is disabled by the client.",
        );
  ...
  it("negotiate called again when transport fails to start and falls back", async () => {
    await VerifyLogger.run(
      async (loggerImpl) => {
        ...
        const options: IHttpConnectionOptions = {
          EventSource: () => {
            throw new Error("Don't allow ServerSentEvents.");
          },
          WebSocket: () => {
            throw new Error("Don't allow Websockets.");
          },
          ...commonOptions,
          httpClient: new TestHttpClient()
            .on("POST", () => {
        ...
        async (logger) => {
          ...
          try {
            ...
            await expect(startPromise).rejects.toThrow(
              "We don't care how this turns out",
            );
          } finally {
            await connection.stop();
          }
  ...
  eachTransport((requestedTransport: HttpTransportType) => {
    it(`cannot be started if requested ${HttpTransportType[requestedTransport]} transport not available on server`, async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        negotiateResponse.availableTransports = negotiateResponse.availableTransports!.filter(
          (f) => f.transport !== HttpTransportType[requestedTransport],
        );
        ...
        await expect(connection.start(TransferFormat.Text)).rejects.toThrow(
          `Unable to connect to the server with any of the available transports. ${negotiateResponse.availableTransports[0].transport} failed: Error: '${negotiateResponse.availableTransports[0].transport}' is disabled by the client.` +
    ...
    it(`cannot be started if server's only transport (${HttpTransportType[requestedTransport]}) is masked out by the transport option`, async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        negotiateResponse.availableTransports = negotiateResponse.availableTransports.filter(
          (r) => r.transport === HttpTransportType[requestedTransport],
        );
        ...
        try {
          ...
          expect(e.message).toBe(
            `Unable to connect to the server with any of the available transports. ${HttpTransportType[requestedTransport]} failed: Error: '${HttpTransportType[requestedTransport]}' is disabled by the client.`,
          );
        }
  ...
  it("redirects to url when negotiate returns it", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      try {
        ...
        expect(httpClient.sentRequests[1].url).toBe(
          "https://another.domain.url/chat/negotiate?negotiateVersion=1",
        );
        expect(httpClient.sentRequests[2].url).toMatch(
          /^https:\/\/another\.domain\.url\/chat\?id=0rge0d00-0040-0030-0r00-000q00r00e00/i,
        );
        expect(httpClient.sentRequests[3].url).toMatch(
          /^https:\/\/another\.domain\.url\/chat\?id=0rge0d00-0040-0030-0r00-000q00r00e00/i,
        );
      } finally {
        await connection.stop();
  ...
  it("fails to start if negotiate redirects more than 100 times", async () => {
    await VerifyLogger.run(async (logger) => {
      const httpClient = new TestHttpClient().on("POST", /\/negotiate/, () => ({
      ...
      const options: IHttpConnectionOptions = {
        ...
        httpClient: new TestHttpClient()
          ...
          .on("GET", () => {
            ...
            if (httpClientGetCount === 1) {
              if (
                (connection as any).transport.onreceive &&
                (connection as any).transport.onclose
              ) {
                handlersSet = true;
              }
              // First long polling request must succeed so start completes
  ...
  it("negotiateVersion query string not added if already present", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      const connection = new HttpConnection(
        "http://tempuri.org?negotiateVersion=42",
        options,
      );
      try {
        const startPromise = connection.start(TransferFormat.Text);

        expect(await connectUrl).toBe(
          "http://tempuri.org?negotiateVersion=42&id=42",
        );
  ...
  it("fallback changes connectionId property", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestWebSocket.webSocket.onerror(new TestEvent() as any);
      ...
      const captureLogger: ILogger = {
        log: (logLevel: LogLevel, message: string) => {
          if (
            ...
            message.search("data of length") > 0
          ) {
            sentMessage = message;
          }
  ...
  it("send after restarting connection works", async () => {
    await VerifyLogger.run(async (logger) => {
      ...
      TestWebSocket.webSocket.onopen(new TestEvent() as any);
      await startPromise;
  ...
  describe(".constructor", () => {
    it("throws if no Url is provided", async () => {
      ...
      expect(() => new (HttpConnection as any)()).toThrowError(
    ...
    it("uses EventSource constructor from options if provided", async () => {
      await VerifyLogger.run(
        async (logger) => {
          ...
          await expect(connection.start(TransferFormat.Text)).rejects.toEqual(
            new Error(
              "Unable to connect to the server with any of the available transports. ServerSentEvents failed: Error: EventSource constructor called.",
            ),
          );
  ...
  describe("startAsync", () => {
    it("throws if an unsupported TransferFormat is provided", async () => {
      await VerifyLogger.run(async (logger) => {
        ...
        await expect(connection.start(42)).rejects.toThrow(
          "Unknown transferFormat value: 42.",
        );
      });
    });
...
describe("TransportSendQueue", () => {
  ...
  it("concatenates buffered ArrayBuffer", async () => {
    ...
    expect(sendMock.mock.calls[1][0]).toEqual(
      new Uint8Array([7, 8, 10, 12, 14]).buffer,
    );
  ...
  it("rejects pending promises on stop", async () => {
    ...
    const transport = new TestTransport();
    const sendMock = jest.fn().mockImplementationOnce(async () => {
      await promiseSource;
    });
    transport.send = sendMock;
